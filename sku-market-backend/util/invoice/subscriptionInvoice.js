...
const fs = require('fs');
const { formatCurrency, convertCurrency } = require('./currency');
...
doc.text('Not Available', 150, 284);
doc.text(customerInfo.companyName, 150, 301);
doc.text(customerInfo?.currency || 'SAR', 150, 318);
...
doc.text(formatCurrency(prev_balance, customerInfo?.currency), 400, 284, { align: 'right' });
doc.text(formatCurrency(total_cost, customerInfo?.currency), 400, 301, { align: 'right' });
doc.text(formatCurrency(total_payment, customerInfo?.currency), 400, 318, { align: 'right' });
doc.text(formatCurrency(ending_balance, customerInfo?.currency), 400, 335, { align: 'right' });
...
doc.text(sellerInfo.email || 'support@skumarkets.com', 435, 800, { goto: 'mailto:support@skumarkets.com' });
...
doc.text(subscriptionInfo?.plan, 40, y, { width: 240 });
doc.text(subscriptionInfo?.plan_type, 285, y, { width: 80 });
doc.text(convertCurrency(subscriptionInfo?.subscription_fees, customerInfo?.currency), 365, y, { width: 80 });
doc.text(convertCurrency(subscriptionInfo?.vat_price, customerInfo?.currency), 445, y, { width: 80 });
doc.text(convertCurrency(subscriptionInfo?.net_cost, customerInfo?.currency), 520, y, { width: 80 });
productNo++;
// });
...
doc.font(fontBold).text("Total:", 320, 401 + (productNo * 20));

doc.font(font).text(formatCurrency(net_cost, customerInfo?.currency), 470, 350 + (productNo * 20), { align: 'right' });
doc.text(formatCurrency(promotion, customerInfo?.currency), 470, 367 + (productNo * 20), { align: 'right' });
doc.text(formatCurrency(discount_price, customerInfo?.currency), 470, 384 + (productNo * 20), { align: 'right' });
// doc.text(formatCurrency(vat_price, customerInfo?.currency), 470, 401 + (productNo * 20), { align: 'right' });
doc.font(fontBold).text(formatCurrency(total_cost, customerInfo?.currency), 470, 401 + (productNo * 20), { align: 'right' });
