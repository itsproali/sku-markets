    ...
    <Stack sx={{ width: '100%', my: 4 }}>
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={2.5}>
          {cardData?.map((item, i) => (
            <Grid item xs={12} lg={item?.title === 'Sales Accounting' ? 12 : 6} key={i}>
              <Box
                sx={{
                  p: 3,
                  width: '100%',
              ...
              >
                ...
                <Grid container spacing={2} sx={{ mt: 2 }}>
                  <Grid item xs={12} sm={item?.title === 'Sales Accounting' ? 8 : 12} sx={{ mb: 2 }}>
                  ...
                  {item?.title !== 'Sales Accounting' ? null : <Grid item xs={12} sm={4}>
                    <Stack
                      direction="row"
                      alignItems="center"
                      sx={{ position: 'relative', mt: { xs: 1, sm: 4 } }}
                    >
                      <RHFTextField
                        name={item?.vat}
                        label="Vat/Tax Rate"
