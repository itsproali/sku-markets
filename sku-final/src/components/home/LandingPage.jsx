  ...
  return (
    <Container sx={{ position: 'relative', zIndex: 1, mt: 15 }} maxWidth="lg" id="#home">
      <Grid
        container
        spacing={2}
        sx={{ mt: { md: `${HEADER.H_MAIN_DESKTOP}px` }, height: { md: '55vh' } }}
      >
          ...
          <div className={style.landingFirstSectionTxt}>
            <div onClick={() => navigate('/signup')} className={style.jumpingPort}>
              ...
              <span style={{ fontSize: '14px' }}>
                {arabic ? 'انطلق لبناء أعمالك' : 'Jump start your portfolio'}{' '}
              </span>
              {arabic ? <HiOutlineArrowNarrowLeft /> : <HiOutlineArrowNarrowRight />}
            </div>
            <div>
              <h1 className={style.Landingh1text}>
                {arabic
                  ? 'منصة متكاملة للرؤى والتحليلات والإحصاءات والإدارة والبيع، مصممة خصيصًأ لتعزيز تجارتك الإلكترونية.'
                  : 'A Complete Insights, Analytics, Statistics, Management & Selling Platform Built to Supercharge Your Ecommerce'}
              </h1>
              <h6 className={style.disFontStyle}>
                {arabic
                  ? 'تتبع ..احصل على إشعار.. أضف.. ثم قم بإدارة عملية البيع لتنمية تجارتك الإلكترونية.'
                  : 'Track it, Get Notified, Added it, Manage to Sell it to grow your Ecommerce'}
              </h6>
              <p>
                {' '}
                {arabic
                  ? 'اس كي يو ماركتس هو المكان الأسهل لتوسيع نطاق التجارتك الإلكترونية.'
                  : 'SKU Markets is the easiest place to scale your Ecommerce'}{' '}
              </p>

              <h6 className={style.disFontStyle}>
                {arabic
                  ? 'مدعوم من الذكاء الإصطناعي، اشترك وابدأ اليوم'
                  : 'Powered By AI, Sign up and get started today'}{' '}
              </h6>
            </div>
            <Button
              sx={{
