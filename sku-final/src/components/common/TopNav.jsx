  ...
  return (
    ...
    >
      <Typography variant="caption">
        {arabic ? 'أسواق اس كي يو الحيّة:' : 'Marketplaces Live SKUs:'}{' '}
        <span style={{ fontWeight: 'bolder' }}> {data?.live}</span>
      </Typography>
      <Typography variant="caption">
        {arabic ? 'التوريد بالمتجر:' : 'Fulfilled by Stores:'}{' '}
        <span style={{ fontWeight: 'bolder' }}>{data?.ff_store}</span>
      </Typography>
      <Typography variant="caption">
        {arabic ? 'التوريد بمنصة البيع:' : 'Fulfilled by MP:'}{' '}
        <span style={{ fontWeight: 'bolder' }}>{data?.ff_mp}</span>
      </Typography>
      <Stack direction="row" alignItems="center" spacing={1}>
        <Typography variant="caption">
          {arabic ? 'الأسواق المهيمنة:' : 'Top Dominance Market:'}
        </Typography>
