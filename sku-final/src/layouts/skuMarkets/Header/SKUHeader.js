...
export default function SKUHeader({ arabic }) {
  ...
  return (
    <AppBar color="transparent" sx={{ boxShadow: 0, backdropFilter: 'blur(20px)' }}>
      <TopNav arabic={arabic} isOffset={isOffset} />
      {isDesktop && (
        <Toolbar
          disableGutters
          sx={{
      ...
      {!isDesktop && (
        <Toolbar disableGutters sx={{ height: { xs: HEADER.H_MOBILE, sm: HEADER.H_MOBILE } }}>
          <SKUHeaderMob arabic={arabic} />
        </Toolbar>
      )}

      <SkuBottomNav arabic={arabic} isOffset={isOffset} />
      {isOffset && <Shadow />}
    </AppBar>
  );
}
