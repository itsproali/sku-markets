...
const FooterBottom = ({ arabic }) => {
  const ITEMS = [
    { label: `${arabic ? 'كن شريكنا' : 'Become A Partner'}`, path: '/become_partner' },
    { label: `${arabic ? 'خطط الأسعار' : 'Pricing Plans'}`, path: '/become_partner' },
    { label: `${arabic ? 'تواصل معنا ' : 'Contact Us'}`, path: '/become_partner' },
    { label: `${arabic ? 'مركز المساعدة ' : 'Help Center'}`, path: '/help_center' },
  ];
